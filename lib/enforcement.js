 ...
 */
export function resolveEnforcementLevel(config) {
    if (!config)
        return 'strict';
    if (config.orchestrateInterceptTools === false)
        return 'none';
    if (config.orchestrateRoundIntercept === false)
        return 'lenient';
    if (config.orchestrateRoundIntercept === true)
        return 'strict';
    if (config.orchestrateEnforcement !== undefined)
        return config.orchestrateEnforcement;
    return 'strict';
}
/**
 ...
 */
export function resolveLayeredEnforcement(mountConfig, userSettings) {
    if (userSettings &&
 ...
 */
export function deriveSwitchesFromEnforcement(enforcement) {
    switch (enforcement) {
        case 'strict':
