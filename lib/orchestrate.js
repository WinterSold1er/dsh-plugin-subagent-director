...
export const ORCHESTRATE_VALID_MODES = ['on', 'off', 'agent-team'];
/**
 ...
 * 3. s.log (if Array)
 */
export function extractSessionEvents(session) {
...
export function detectOrchestrateRequest(text) {
    ...
    if (/^(请问|什么是|怎么|如何|怎样)/i.test(trimmed))
        return undefined;
    // 2. Agent-Team natural language matching
    if (/^(?:请(?!问)|麻烦|麻烦你|帮我|请帮我|我想|我要)?\s*(?:使用\s*(?:agent[- ]team|智能体团队)(?:\s*(?:模式|mode))?|using\s*agent[- ]team|using-agent-team|use\s+agent[- ]team(?:\s+mode)?)(?!.*?(?:注意事项|区别|优缺点|特点|好不好|怎么样|吗|？|\?))(?:\s*[:：]|\s*.*$|$)/i.test(trimmed)) {
        return 'agent-team';
    }
    // 3. Subagents natural language matching
    if (/^(?:请(?!问)|麻烦|麻烦你|帮我|请帮我|我想|我要)?\s*(?:使用\s*(?:子代理|orchestrate)(?:\s*(?:模式|mode))?|using\s*subagents|using-subagents|use\s+subagents|use\s+orchestrate(?:\s+mode)?)(?!.*?(?:注意事项|区别|优缺点|特点|好不好|怎么样|吗|？|\?))(?:\s*[:：]|\s*.*$|$)/i.test(trimmed)) {
        return 'on';
    }
    return undefined;
}
...
export function buildOrchestratorFrame(toolName, enforcement = 'strict') {
    const enforcementSentence = enforcement === 'none'
        ? 'Tool interception is disabled in current settings; strictly follow delegation rules at prompt level.'
 ...
 * @param toolName - the configured model-facing delegation tool name.
 */
export function renderOrchestratorSection(settings, toolName, enforcement) {
    const roles = settings.roles ?? {};
 ...
 */
export function buildAgentTeamFrame(enforcement = 'strict') {
    const enforcementSentence = enforcement === 'none'
 ...
 */
export function renderAgentTeamPrompt(settings, toolName, enforcement = 'strict') {
 ...
 */
export function renderAgentTeamSection(settings, toolName, enforcement) {
    const roles = settings.roles ?? {};
    const hasRoles = Object.values(roles).some((role) => role !== undefined);
    if (!hasRoles)
        return renderOrchestratorUnavailableNotice('spawn_teammate');
...
export function applyOrchestrate(ctx, getSettings, toolName, options) {
    ...
    try {
        ...
        KNOWN_SESSION_EVENT_TYPES.add(AGENT_TEAM_EVENT_TYPE);
    }
    catch (err) {
    ...
    const missing = () => ctx.logger.warn('[orchestrate] sessionProjections service is missing on this host — the /using-subagents command will NOT take effect (no projection registered, orchestrator prompt will not inject). Provide the dsh-session-projection sessionProjections service to enable orchestrator mode.');
    ...
    ctx.inject(['commands'], (injectedCtx) => {
        ...
        const handleSubagentsCommand = (invocation, cmdName) => {
            ...
            if (projections === undefined) {
                ...
                return {
                    kind: 'error',
                    text: `Orchestrator mode was NOT applied: the sessionProjections service is missing on this host, so /${cmdName} has no effect and the orchestrator prompt will not inject. ` +
            ...
            if (session === undefined || typeof session.append !== 'function') {
                return {
                    kind: 'error',
                    text: `Orchestrator mode was NOT applied: this command invocation carries no agent session to append the mode change to.`,
                };
            }
            if (lower === 'off') {
        ...
        const handleAgentTeamCommand = (invocation) => {
            const raw = (invocation.rawInput || '').trim();
            const lower = raw.toLowerCase();
            if (projections === undefined) {
                missing();
                return {
                    kind: 'error',
                    text: `Agent-team mode was NOT applied: the sessionProjections service is missing on this host, so /using-agent-team has no effect and the team lead prompt will not inject. ` +
            ...
            if (session === undefined || typeof session.append !== 'function') {
                return {
                    kind: 'error',
                    text: `Agent-team mode was NOT applied: this command invocation carries no agent session to append the mode change to.`,
                };
            }
            if (lower === 'off') {
                session.append(ORCHESTRATE_EVENT_TYPE, { mode: 'off' });
        ...
        commands.register({
            ...
            description: 'Enter pure-orchestrator mode using subagents — /using-subagents <task> or say 使用子代理. Persistent until /using-subagents off.',
            input: { hint: '<task> | on | off' },
            handler: (inv) => handleSubagentsCommand(inv, 'using-subagents'),
        });
        // 2. Alias: orchestrate (retained for backwards compatibility)
        commands.register({
            name: 'orchestrate',
            description: 'Alias for /using-subagents — enter pure-orchestrator mode. Persistent until /orchestrate off.',
            input: { hint: '<task> | on | off' },
            handler: (inv) => handleSubagentsCommand(inv, 'orchestrate'),
        });
        // 3. New command: using-agent-team
        commands.register({
            name: 'using-agent-team',
            description: 'Enter Agent-Team Lead mode — /using-agent-team <task> or say 使用agent-team. Persistent until /using-agent-team off.',
            input: { hint: '<task> | on | off' },
