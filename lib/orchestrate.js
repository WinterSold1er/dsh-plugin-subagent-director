...
export function detectOrchestrateRequest(text) {
    ...
    if (/^(请(?!问)|麻烦|麻烦你|帮我|请帮我|我想|我要)?\s*使用\s*orchestrate\s*(模式|mode)(?!.*?(?:注意事项|区别|优缺点|特点|好不好|怎么样|吗|？|\?))(\s*[:：]|\s*.*$)/i.test(trimmed)) {
        return 'on';
    }
...
export function buildOrchestratorFrame(toolName, enforcement = 'strict') {
    const enforcementSentence = 'This contract is ENFORCED at the tool level: any disallowed tool you call is blocked by the harness (you will see a BLOCKED result), and retrying it will keep failing.';
