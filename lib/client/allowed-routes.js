...
export function allowedRouteLabel(route) {
...
export function providerNames(routes) {
    const seen = new Set();
    const names = [];
    for (const route of routes) {
        if (seen.has(route.provider))
            continue;
...
export function modelsForProvider(routes, provider) {
    return buildRouteOptions(routes).filter((option) => option.provider === provider);
}
//# sourceMappingURL=allowed-routes.js.map