...
export function resolveRoleId(customId, name, existing, prefix = 'role') {
 ...
 */
export function validateRoleSubmission(idInput, displayName, existing, currentId, description) {
 ...
 */
export function renameRoleOps(oldId, newId, before, draft, defaultRole) {
    if (oldId === newId) {
    ...
    const ops = [
        { op: 'unset', path: [ROLES_PATH[0], oldId] },
        {
            op: 'set',
    ...
    if (defaultRole === oldId) {
        ops.push({ op: 'set', path: ['defaultRole'], value: newId });
    }
    return ops;
}
/**
...
export function interceptSwitchesOps(before, edits) {
    const ops = [];
    ...
    if (before.orchestrateInterceptTools !== nextIntercept) {
        ops.push({ op: 'set', path: ['orchestrateInterceptTools'], value: nextIntercept });
    }
    ...
    if (roundInterceptEdited) {
        ...
        if (before.orchestrateRoundIntercept !== nextRound) {
            ops.push({ op: 'set', path: ['orchestrateRoundIntercept'], value: nextRound });
        }
    }
    ...
    if (before.orchestrateEnforcement !== nextEnforcement) {
        ops.push({ op: 'set', path: ['orchestrateEnforcement'], value: nextEnforcement });
    }
    return ops;
}
