...
import { SUBAGENT_DIRECTOR_RPC_VIEW, SUBAGENT_DIRECTOR_RPC_MUTATE, SUBAGENT_DIRECTOR_RPC_CLOSE, SUBAGENT_DIRECTOR_RPC_MODEL, SUBAGENT_DIRECTOR_RPC_TOOLS, SUBAGENT_DIRECTOR_RPC_CATALOG, } from './bridge-contract.js';
 ...
 */
export function directorCatalogOk(settings) {
    if (settings === undefined) {
        return { ok: true, value: { modelSelectionEnabled: false, allowedRoutes: [] } };
    }
    try {
        const section = settings.get('subagent-model-selection');
...
export async function directorMutate(mutate, describe, ns, ops, expectedRevision) {
    ...
    const descriptor = describe({ redactSecrets: true }).find((candidate) => candidate.ns === ns);
...
async function dispatchBridgeEndpoint(deps, endpoint, payload) {
    ...
    if (endpoint === SUBAGENT_DIRECTOR_RPC_CATALOG) {
        return directorCatalogOk(deps.settings);
    }
