 ...
 */
export const AGENT_TEAM_MANAGEMENT_TOOLS = [
    'spawn_teammate',
    'send_message',
    'list_agents',
    'wait_agent',
 ...
 */
export function resolveMainAgentAllowedTools(toolName, settings, readOnlyFallback) {
    ...
    if (defaultRole?.toolFilter?.allow &&
...
export function createOrchestrateToolGuard(deps) {
    ...
    const blocked = (rawInnerName, isAgentTeam) => {
        if (isAgentTeam) {
            return (`BLOCKED: orchestrate mode is ON (Agent-Team). Tool \`${rawInnerName}\` is blocked by tool permission settings for the Lead agent. ` +
        ...
        return (`BLOCKED: orchestrate mode is ON for this session. Tool \`${rawInnerName}\` is blocked by the tool permission settings for the Main Agent (default role). ` +
    ...
    return (exec) => {
        ...
        if (origin === 'subagent' || origin === 'teammate' || delegationDepth > 0)
            return undefined;
